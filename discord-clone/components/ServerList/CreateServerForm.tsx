...
export default function CreateServerForm(): JSX.Element {
    ...
    function createClicked(){
        if (!videoClient){
            console.log('[CreateServerForm] Video client not avaible')
            return;
        }
        createServer(
            client,
    ...
    function userChanged(user: UserObject, checked: boolean): void {
        setFormData({
            ...formData,
            users: checked
                ? [...formData.users, user]
                : formData.users.filter((thisUser) => thisUser.id !== user.id)
        });
    }

    return (
        <dialog className="absolute z-100 space-y-2 rounded-xl" ref={dialogRef}>
            ...
            <form method="dialog" className="flex flex-col space-y-2 px-6">
                ...
                <label className="labelTitle" htmlFor="serverImage">Image URL</label>
                <div className="flex items-center bg-gray-100 rounded">
                    <span className="text-2xl p-2 text-gray-500">#</span>
                    <input
                        type="text"
